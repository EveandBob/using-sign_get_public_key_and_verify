...
def verif_sign(msg,sign,ID,P):
    ...
    data = ENTL.to_bytes(2, byteorder='big', signed=False) + int_to_bytes(ID) + int_to_bytes(a) + int_to_bytes(
        b) + int_to_bytes(Gx) + int_to_bytes(Gy) + int_to_bytes(PA[0]) + int_to_bytes(PA[1])
    ZA = int(sm3.sm3_hash(list(data)), 16)
    M_ = int_to_bytes(ZA) + M
    e = int(sm3.sm3_hash(list(M_)), 16)
    t = (r + s) % n
...
def verif_sign_USE_R_S_get_PA(msg,sign,ID):
    def get_y(x):
        return get_Qr(x*x*x+a*x+b,p)
    ...
    M_ = int_to_bytes(ZA) + M
    e = int(sm3.sm3_hash(list(M_)), 16)
    PA1,PA2=USE_R_S_get_PA(sign,e)
    t=(r+s)%n
    if t==0 :
        return False
    SG=calculate_np(Gx, Gy, s, a, b, p)
    tG=calculate_np(PA1[0], PA1[1], t, a, b, p)
    xx,yy=calculate_p_q(SG[0],SG[1],tG[0],tG[1],a,b,p)
    R=(e+xx)%n
    if R==sign[0]:
        return True
    tG = calculate_np(PA2[0], PA2[1], t, a, b, p)
    xx, yy = calculate_p_q(SG[0], SG[1], tG[0], tG[1], a, b, p)
    R = (e + xx) % n
    if R == sign[0]:
        return True

print("通过r和s来获取PA并且来验证签名")
print(verif_sign_USE_R_S_get_PA(msg,sm3_sign(msg,IDA),IDA))
